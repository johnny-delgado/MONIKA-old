...
layer1_size = 50 #the size of the first hidden layer
...
def setTeachingArray(numOfDataPoints):
	...
	for row in xrange(len(Days)):
		...
		for i in xrange(numOfDataPoints): #make this many data points per day
...
setTeachingArray(60) #how many data points per day in teaching array
synapse_0, synapse_1 = teachMonika(X, Y, synapse_0, synapse_1, 5000, .005) #number of iterations through training array, alpha value (change per iteration) #https://stackoverflow.com/questions/11690333/is-it-possible-to-return-two-lists-from-a-function-in-python
