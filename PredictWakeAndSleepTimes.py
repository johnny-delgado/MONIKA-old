#corresponds to TrainPredictWakeAndSleepTimes.py
#18 June 2018 1231
...
def predictionOutput(dotw, doty):
	...
	layer_0 = [1, Sun, Mon, Tues, Wed, Thur, Fri, Sat, sin_doty(doty), cos_doty(doty)]
...
def predictStateTime(dotw, doty, year, state, prob, minHour, maxHour): 
	...
	for h in my_range(minHour, minHour + hourRange, 0.05):
		...
		if(h > 24): #account for 
			...
			dotw += 1
			doty += 1
		chanceInState = isJohnnySleepingAt(hour, dotw, doty, year)[state]
...
def predictNextWeek(startDotw , startDoty, year, state, prob, minHour, maxHour): # dotw (of start day), doty (of start day)
	for day in range(0, 7): #[0,7) | 0 = start of week we're asking about
		print predictStateTime(startDotw+day, startDoty+day, year, state, prob, minHour, maxHour)


'''print predictWakeTime(5, 164, 2018, 0, .5)
print predictWakeTime(6, 165, 2018, 0, .5)
print predictWakeTime(7, 166, 2018, 0, .5)
print predictWakeTime(8, 167, 2018, 0, .5)
print predictWakeTime(9, 168, 2018, 0, .5)
print predictWakeTime(10, 169, 2018, 0, .5)
print predictWakeTime(11, 170, 2018, 0, .5)
'''

#print "wake times:"
#predictNextWeek(1, 167, 2018, 0, .5, 7, 20)

#print "bed times:"
#predictNextWeek(1, 167, 2018, 2, .2, 20, 6)


print predictionOutput(1, 167)
